...
% check the rank of each sub-block
rankPSD_v1 = zeros(length(X.blk),1);
rankPSD_v2 = zeros(length(X.blk),1);
for i = 1: length(X.blk)
    rankPSD_v1(i) = rank(X.blk{i});
    rankPSD_v2(i) = length(find( abs(eig(X.blk{i})) > 1e-5));
end
info.rankPSD_v1 = rankPSD_v1;
info.rankPSD_v2 = rankPSD_v2;
...
elseif strcmpi(opts.solver,'rankd')
